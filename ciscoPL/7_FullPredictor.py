...
#python ./ciscoPL/7_FullPredictor.py folderName testName
#python ./ciscoPL/7_FullPredictor.py test1 data11
...
for s in signalClusters:
    try:	
        ...
        tbadRules[s], tbadRuleWord[s], tbadRuleCounter[s], tbadRuleWordPos[s], tbadRuleWordR[s], tbadRuleCounterR[s], tbadRuleWordPosR[s] = getBadRules(clean,maps,keys,tolerance,limit,probRisk)
        
        rrule = []
        for w in grouped[s]:
            rrule = rrule + [maps[w]]
        tfulllist[s] = ''.join(list(grouped[s]))
        
        totalRisk[s] = probRisk
        tmaps[s] = maps
        tmaps2[s] = maps2
        tclean[s] = clean
        tkeys[s] = keys
        tpastword[s] = keys
        tsize[s] = size
        ttolerance[s] = tolerance
    except:
        print('No grammar file for',s)    
        
if(runPredictor):
    for i, word in enumerate(grouped['sym0']):
        a=0
        currRisk = 0
        currRisk2 = 0
        currRisk3 = 0
        for s in signalClusters:
            try:	
                #////////////////// prep data /////////////
                word = grouped[s][i]
                probRisk = totalRisk[s]
                maps = tmaps[s]
                maps2 = tmaps2[s]
                clean = tclean[s]
                keys = tkeys[s]
                size = tsize[s]
                pastword = tpastword[s]
                tolerance = ttolerance[s]
                fulllist = tfulllist[s]

                badRules = tbadRules[s]
                badRuleWord = tbadRuleWord[s]
                badRuleCounter = tbadRuleCounter[s]
                badRuleWordPos = tbadRuleWordPos[s]
                badRuleWordR = tbadRuleWordR[s]
                badRuleCounterR = tbadRuleCounterR[s]
                badRuleWordPosR = tbadRuleWordPosR[s]
                ...
                if word in tolerance:
                    ...
                    currRisk = currRisk + float(certainty)/100.0
                    if check:
                ...
                tbadRules[s] = badRules
                tbadRuleWord[s] = badRuleWord
                tbadRuleCounter[s] = badRuleCounter
                tbadRuleWordPos[s] = badRuleWordPos
                tbadRuleWordR[s] = badRuleWordR
                tbadRuleCounterR[s] = badRuleCounterR
                tbadRuleWordPosR[s] = badRuleWordPosR
                ...
                if minW < 999:# and minW > 0:
                    ...
                    currRisk3 = currRisk3 + float(certainty)/100.0
                    softlim = limit
                    for d in range(denom):
                        ...
                        if k > -1:
                            num = num+1
                    currRisk2 = currRisk2 + round(100*num/denom)
        ...
        probs = round(100*currRisk/len(tmaps.keys()))
        probs2 = round(currRisk2/len(tmaps.keys()))
        probs3 = round(100*currRisk3/len(tmaps.keys()))
        if check:
            badRuleCounter = [-1]*len(badRuleCounter)
        check = False
        ...
        if (probs == 100):
            risk = 2
            print('Time',i,"ERR: Anomalous state currently active")
        else:
            risk = 1
            print('Time',i,"WHI: State with",probs,'% chance of being anomaouls, currently active')
        if (probs3 > 0):
            fod = "     "+" "*len(str(i))
            if (probs2 > 0):
                print(fod,"WLO: State with",probs3,'% anomaly risk, in the next',softlim,'words with',probs2,'% probability')
            else:
                print(fod,"WLO: State with",probs3,'% anomaly risk, in the next',softlim,'words with minimal chance')
